...
export default function AnalyticsPage() {
  ...
  return (
    <div className="space-y-32">
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            {!data ? (
              ...
              <p className="text-3xl font-bold text-admin-text tabular-nums">{totalViews.toLocaleString('fr-FR')}</p>
            )}
            <p className="text-xs text-admin-text-muted mt-1.5 uppercase tracking-wider font-medium">
              Vues totales ({range} jours)
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            ...
            {!data ? (
              ...
              <p className="text-3xl font-bold text-admin-text tabular-nums">{avgViews.toLocaleString('fr-FR')}</p>
            )}
            <p className="text-xs text-admin-text-muted mt-1.5 uppercase tracking-wider font-medium">
              Moyenne / jour
            </p>
            ...
            <div className="relative h-64">
              ...
              <div className="relative h-full flex items-end gap-px pt-2 pb-6">
                {analytics.map((day, idx) => (
                  <div key={`${day.date}-${idx}`} className="flex-1 flex flex-col items-center group/bar relative h-full justify-end">
                    ...
                    <div
                      className="w-full rounded-t-md bg-gradient-to-t from-amber-500 to-amber-400 transition-all duration-300 hover:from-amber-400 hover:to-amber-300 min-h-[4px]"
      ...
      <Card>
        <CardHeader>
          <CardTitle>Articles les plus performants</CardTitle>
        </CardHeader>
